...
static void decimal_to_rational(const F64& value, I32& rational_part, I32& numerator, I32& denominator, const I32 max_den = MAX_DENOMINATOR)
{
    ...
    while (static_cast<I64>(mat.m10) * (ai = static_cast<I64>(x)) + static_cast<I64>(mat.m11) <= static_cast<I64>(max_den)) {
        ...
        if (x == static_cast<F64>(ai)) break;
        x = 1.0 / (x - static_cast<F64>(ai));
        if (x > static_cast<F64>(0x7FFFFFFF)) break; 
    }
    ...
    I64 rational_part64 = mat.m00 / mat.m10;

    if (rational_part64 > std::numeric_limits<I32>::max() || rational_part64 < std::numeric_limits<I32>::min()) {
        rational_part = 0;
        numerator = static_cast<I32>((mat.m00 > 0 ? 1 : -1) * std::numeric_limits<I32>::max());
        denominator = 1;
        return;
    }

    rational_part = static_cast<I32>(rational_part64);

    if (rational_part != 0) {
        I64 numerator64 = mat.m00 - rational_part * static_cast<I64>(mat.m10);

        if (numerator64 > std::numeric_limits<I32>::max() || numerator64 < std::numeric_limits<I32>::min()) {
            numerator = 0;
        }
        else {
            numerator = static_cast<I32>(numerator64);
        }

        rational_part *= sign;
        denominator = static_cast<I32>(mat.m10);
        return;
    }

    rational_part = 0;

    if (mat.m00 > std::numeric_limits<I32>::max() || mat.m00 < std::numeric_limits<I32>::min()) {
        numerator = (sign > 0) ? std::numeric_limits<I32>::max() : std::numeric_limits<I32>::min();
    }
    else {
        numerator = static_cast<I32>(sign * mat.m00);
    }
